...
function stockAvailabilityParams(template) {
  const filter = template && template.filter_options && typeof template.filter_options === 'object'
    ? template.filter_options
    : {};
  const has = (key) => Object.prototype.hasOwnProperty.call(filter, key);
  const textOr = (key, fallback) => {
    if (!has(key)) return fallback;
    return String(filter[key] == null ? '' : filter[key]).trim();
  };
  return {
    ioOptimized: has('io_optimized') ? (filter.io_optimized ? 'optimized' : 'none') : 'optimized',
    systemDiskCategory: textOr('system_disk_category', 'cloud_essd'),
    dataDiskCategory: textOr('data_disk_category', 'cloud_essd'),
    networkCategory: textOr('network_category', 'vpc'),
  };
}
...
function stockBillingParams(template) {
  ...
  return {
    ...
    ...stockAvailabilityParams(template),
  };
}
...
function buildRunHardwareStockPath(target, tenantId, containerImageId, billing) {
  ...
  if (bill.ioOptimized) params.set('IoOptimized', bill.ioOptimized);
  if (bill.systemDiskCategory) params.set('SystemDiskCategory', bill.systemDiskCategory);
  if (bill.dataDiskCategory) params.set('DataDiskCategory', bill.dataDiskCategory);
  if (bill.networkCategory) params.set('NetworkCategory', bill.networkCategory);
