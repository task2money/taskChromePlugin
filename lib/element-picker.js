...
const DEFAULT_ADJUST_PROMPT = '请分析制定方案并实施';
...
function pierceSameOriginIframe(iframe, clientX, clientY, options = {}) {
  ...
  try {
    ...
    const err = new Error(typeof tx === 'function' ? tx('pickCrossOriginIframe') : '跨域 iframe 无法选择内部元素');
...
function snapshotDisjointSelection(elements, options = {}) {
  ...
  return {
    ...
    // 单行：cs1(all_frames) 无 i18n-tx.js，只能内联兜底；门禁按行扫描。
    label: typeof tx === 'function' ? tx('pickMultiSelectLabel', { count: snaps.length }) : `多选 ×${snaps.length}`,
    elements: snaps,
    tagName: '*',
    id: '',
    className: '',
...
function validateAdjustment(adjustment) {
  ...
  if (t.length > 2000) {
    return typeof tx === 'function' ? tx('pickAdjustTooLong') : '调整期望过长（最多 2000 字）';
  }
  return null;
}
...
function validateScreenshotDataUrl(dataUrl) {
  ...
  if (!/^data:image\/(png|jpeg|jpg|webp);base64,/i.test(s)) {
    return typeof tx === 'function' ? tx('pickScreenshotFormatInvalid') : '截图格式无效（需要 data:image/*;base64）';
  }
  if (s.length > MAX_SCREENSHOT_CHARS) {
    return typeof tx === 'function' ? tx('pickScreenshotTooLarge', { max: MAX_SCREENSHOT_CHARS }) : `截图过大（>${MAX_SCREENSHOT_CHARS} 字符），请取消勾选后重试`;
  }
  return null;
}
...
function validateScreenshotUrl(url) {
  ...
  if (/^data:/i.test(s)) {
    return typeof tx === 'function' ? tx('pickScreenshotUseUploadedUrl') : '截图应使用已上传的 http(s) URL，请勿内嵌 data URL';
  }
  if (!/^https?:\/\//i.test(s)) {
    return typeof tx === 'function' ? tx('pickScreenshotUrlInvalid') : '截图 URL 无效（需要 http/https）';
  }
  if (s.length > 2000) {
    return typeof tx === 'function' ? tx('pickScreenshotUrlTooLong') : '截图 URL 过长';
  }
  return null;
}
...
/** 将 iframe 选择器前缀接到 cssPath 前（`iframe >>> … >>> path`） */
function prefixCssPathWithFrames(cssPath, framePrefixes) {
  const path = String(cssPath || '').trim();
  if (!path || !framePrefixes?.length) return path;
  return `${framePrefixes.join(' >>> ')} >>> ${path}`;
}

/**
 * 给 snapshot 的聚合 cssPath 与 elements[].cssPath 统一加 frame 前缀。
 * 不相交多选常把聚合 cssPath 置空，路径只在 elements 上。
 */
function applyFramePrefixesToSnapshot(snapshot, framePrefixes) {
  if (!snapshot || !framePrefixes?.length) return snapshot;
  const next = { ...snapshot };
  if (next.cssPath) next.cssPath = prefixCssPathWithFrames(next.cssPath, framePrefixes);
  if (Array.isArray(next.elements)) {
    next.elements = next.elements.map((el) => ({
      ...el,
      cssPath: prefixCssPathWithFrames(el.cssPath, framePrefixes),
    }));
  }
  return next;
}
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    MAX_TEXT, MAX_OUTER, MAX_SCREENSHOT_CHARS, SCREENSHOT_MAX_WIDTH,
    DEFAULT_ADJUST_PROMPT, truncateText, buildElementLabel, CSS_escapeId,
    CSS_escapeClass, normalizeClassList, buildCssPath,
    getParentCrossingShadow, getOpenOrClosedShadowRoot, hitTestInRoot,
    deepElementFromPoint, buildAncestorChain, resolveComposedElement,
    pierceSameOriginIframe, isLikelyUaShadowHost, detectUaShadowOpaque,
    normalizeFrameUrl, urlsLikelySameFrame, findIframeElementByUrl,
    buildFramePathFromRoot, accumulateFrameViewportRect,
    getChildElementIndex, getNthOfType, unionClientRects,
    toggleDisjointSelection, snapshotDisjointSelection, snapshotElement,
    validateAdjustment, validateScreenshotDataUrl, validateScreenshotUrl,
    formatElementAdjustmentBlock, appendElementAdjustmentToDescription,
    prefixCssPathWithFrames, applyFramePrefixesToSnapshot,
  });
}
}
