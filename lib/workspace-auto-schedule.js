...
function resolveQueuedAutoRunHint() {
  return tx('panelQueuedAutoRunHint');
}
...
function showFetchError(errorEl, err) {
  if (!errorEl) return;
  const message = err && err.message ? String(err.message) : tx('panelWorkspaceScheduleLoadFailed');
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    isWorkspaceAutoScheduleEnabled, shouldShowCreateTaskQueuedAutoRun,
    resolveQueuedAutoRunHint, appendQueuedAutoRunToCreatePayload,
    applyCreateTaskQueuedVisibility, setResultErrorClass, clearFetchError,
    showFetchError,
  });
}
}
