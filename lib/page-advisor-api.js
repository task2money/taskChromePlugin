...
const PageAdvisorAPI = (() => {
  ...
  const ENDPOINTS = {
    ...
    systemPromptSkills: '/api/page-advisor/v1/system-prompt-skills/',
    promptSkills: '/api/page-advisor/v1/tenant_id/{tenantId}/workspace_id/{workspaceId}/prompt-skills/',
    // OPT-20260922-004: 修订历史 / 回滚。
    promptSkillRevisions:
    ...
    // OPT-20260922-035: 直连 LLM 失败的一次性观测上报（服务端结构化日志含同一 trace_id）。
    clientLlmFailures: '/api/page-advisor/v1/tenant_id/{tenantId}/client-llm-failures/',
  };

  // 与 taskPageAdvisor/src/client_llm_failure.go 的 clientLlmFailureSnippetMaxRunes 对齐。
  const LLM_FAILURE_SNIPPET_MAX_CHARS = 500;
  const DIRECT_LLM_FAILED_CODE = 'PLUGIN_DIRECT_LLM_FAILED';
  ...
  function requireSession(session) {
    ...
    if (!baseUrl || !token) {
      const err = new Error(tx('paApiNotInitialized'));
      err.errorCode = 'PA_SESSION_MISSING';
      throw err;
    }
  ...
  function sessionFromApi() {
    ...
    return requireSession({ baseUrl: api.getBaseUrl(), token: api.getToken() });
  }
  ...
  // buildClientLlmFailurePayload 是纯函数：截断 + 抹掉本机 Key，便于单测断言
  // 出站报文不含密钥（OPT-20260922-035）。服务端只记审计字段，不上报整页正文。
  function buildClientLlmFailurePayload({ traceId, errorCode, errorMessage, model, contentSnippet, apiKey } = {}) {
    const redact = (typeof PageAdvisorLLM !== 'undefined'
      && typeof PageAdvisorLLM.redactSnippet === 'function')
      ? (s, k) => PageAdvisorLLM.redactSnippet(s, k, LLM_FAILURE_SNIPPET_MAX_CHARS)
      : (s, k) => {
        let out = String(s || '').trim().slice(0, LLM_FAILURE_SNIPPET_MAX_CHARS);
    ...
    return {
      ...
      error_message: redact(errorMessage, apiKey),
      model: String(model || '').trim(),
      content_snippet: redact(contentSnippet, apiKey),
    };
  }

  /**
   * 直连 LLM 失败的一次性观测上报：best-effort，任何失败都吞掉返回 false，
   * 绝不能因为观测再抛错而盖住原始失败（OPT-20260922-035）。
   */
  async function reportClientLlmFailure(tenantId, payload, sessionOverride) {
    const tid = String(tenantId || '').trim();
    const traceId = String(payload?.trace_id || '').trim();
    if (!tid || !traceId) return false;
    try {
      const session = sessionOverride || sessionFromApi();
      const path = APIHttp.buildPath(ENDPOINTS.clientLlmFailures, { tenantId: tid });
      await requestWithHeaders(session, 'POST', path, payload);
      return true;
    } catch (e) {
      console.warn('[taskChromePlugin] client_llm_failure_report_failed', {
  ...
  /**
   * 直连失败上报的一站式入口（OPT-20260922-035）：未登录 / 无 tenant / 缺 traceId
   * 一律跳过并返回 false，任何异常都不冒泡——观测不得盖住用户侧的原始失败。
   * @param {{ sessionOk?: boolean, tenantId?: string, err?: Error, traceId?: string, model?: string, apiKey?: string }} deps
   */
  async function reportDirectLlmFailure({ sessionOk, tenantId, err, traceId, model, apiKey } = {}, sessionOverride) {
    if (!sessionOk) return false;
    const payload = buildClientLlmFailurePayload({
      traceId,
      errorCode: DIRECT_LLM_FAILED_CODE,
      errorMessage: err?.message || '',
      contentSnippet: err?.snippet || '',
      model,
      apiKey,
    });
    return reportClientLlmFailure(tenantId, payload, sessionOverride);
  }
  ...
  async function getSystemPromptSkills(sessionOverride) {
    const session = sessionOverride || sessionFromApi();
    return requestWithHeaders(session, 'GET', ENDPOINTS.systemPromptSkills, null, null);
  }

  async function getPromptSkills(tenantId, workspaceId, sessionOverride) {
    const session = sessionOverride || sessionFromApi();
    const tid = String(tenantId || '').trim();
    const wid = String(workspaceId || '').trim();
    if (!tid || !wid) throw new Error(tx('paSkillNeedWorkspace'));
    const path = APIHttp.buildPath(ENDPOINTS.promptSkills, { tenantId: tid, workspaceId: wid });
    return requestWithHeaders(session, 'GET', path, null, null);
  }

  async function putPromptSkills(tenantId, workspaceId, payload, idempotencyKey, sessionOverride) {
    const session = sessionOverride || sessionFromApi();
    const tid = String(tenantId || '').trim();
    const wid = String(workspaceId || '').trim();
    if (!tid || !wid) throw new Error(tx('paSkillNeedWorkspace'));
    ...
    if (!key) throw new Error(tx('swMissingIdempotencyKey'));
    const path = APIHttp.buildPath(ENDPOINTS.promptSkills, { tenantId: tid, workspaceId: wid });
    return requestWithHeaders(session, 'PUT', path, payload, {
      'Idempotency-Key': key,
    });
  }

  /** 工作空间 Skill 的修订历史（最新在前）；返回服务端的 items 数组。 */
  async function listPromptSkillRevisions(tenantId, workspaceId, sessionOverride) {
    const session = sessionOverride || sessionFromApi();
    const tid = String(tenantId || '').trim();
    const wid = String(workspaceId || '').trim();
    if (!tid || !wid) throw new Error(tx('paSkillNeedWorkspace'));
    const path = APIHttp.buildPath(ENDPOINTS.promptSkillRevisions, { tenantId: tid, workspaceId: wid });
    const body = await requestWithHeaders(session, 'GET', path, null, null);
    return Array.isArray(body?.items) ? body.items : [];
  }

  /**
   * 回滚到指定修订；baseRevision 非空时服务端做 CAS（另一端已改则 409）。
   * 返回回滚后的当前文档。
   */
  async function restorePromptSkillRevision(tenantId, workspaceId, revision, baseRevision, idempotencyKey, sessionOverride) {
    const session = sessionOverride || sessionFromApi();
    const tid = String(tenantId || '').trim();
    const wid = String(workspaceId || '').trim();
    const rev = String(revision || '').trim();
    if (!tid || !wid) throw new Error(tx('paSkillNeedWorkspace'));
    if (!rev) throw new Error(tx('paSkillHistoryFailed'));
    const key = String(idempotencyKey || '').trim();
    if (!key) throw new Error(tx('swMissingIdempotencyKey'));
    const path = APIHttp.buildPath(ENDPOINTS.promptSkillRestore, {
      tenantId: tid,
      workspaceId: wid,
      revision: rev,
    });
    ...
    return requestWithHeaders(session, 'POST', path, payload, { 'Idempotency-Key': key });
  }
  ...
  return {
    ...
    dismissSuggestion,
    pollSuggestJob,
    getSystemPromptSkills,
    getPromptSkills,
    putPromptSkills,
    listPromptSkillRevisions,
    restorePromptSkillRevision,
    requestWithHeaders,
    pickJobTraceId,
    buildClientLlmFailurePayload,
    reportClientLlmFailure,
    reportDirectLlmFailure,
  };
})();
