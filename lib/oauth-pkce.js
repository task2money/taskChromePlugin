/**
 * OAuth2 + PKCE 授权码流（OPT-20260808-024）
 *
 * 对接 taskAuth /api/oidc/* 端点：
 * - authorize: GET {base}/api/oidc/authorize?client_id=chrome-extension&redirect_uri=chrome-extension://<id>/oauth-callback.html&response_type=code&scope=openid&state=...&code_challenge=...&code_challenge_method=S256
...
(function initOAuthPKCE(global) {
  ...
  async function exchangeCodeForToken(opts) {
    ...
    try {
      ...
      throw new Error(`OAuth token 交换网络失败: ${err && err.message}`);
    }
    ...
    if (!resp.ok) {
      ...
      throw new Error(`OAuth token 交换失败 (${resp.status}): ${detail || text || '未知错误'}`);
    }
