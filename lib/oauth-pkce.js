...
(function initOAuthPKCE(global) {
  ...
  // Secret-Hardcode-OK: public OAuth native client (PKCE); value is the registered public client secret in conf/auth bootstrap, not a confidential credential
  ...
  async function postTokenRequest(base, body, label) {
    ...
    try {
      ...
      throw new Error(tx('oauthNetworkFailed', { label, reason: err && err.message }));
    }
    ...
    if (!resp.ok) {
      ...
      throw new Error(
        tx('oauthRequestFailed', {
          label,
          status: resp.status,
          reason: detail || text || tx('swUnknownError'),
        }),
      );
    }
  ...
  async function exchangeCodeForToken(opts) {
    ...
    return postTokenRequest(base, body, tx('oauthLabelTokenExchange'));
  }
  ...
  async function refreshAccessToken(opts) {
    ...
    return postTokenRequest(base, body, tx('oauthLabelTokenRefresh'));
  }
  ...
  async function fetchUserInfo(baseUrl, accessToken) {
    ...
    try {
      ...
      throw new Error(tx('oauthUserinfoNetworkFailed', { reason: err && err.message }));
    }
    const text = await resp.text().catch(() => '');
    if (!resp.ok) {
      throw new Error(
        tx('oauthUserinfoFailed', { status: resp.status, reason: text || tx('swUnknownError') }),
      );
    }
    return JSON.parse(text || '{}');
  }
