...
function parseSimpleYaml(text) {
  ...
  for (const rawLine of lines) {
    ...
    if (!kvMatch) {
      throw new Error(tx('aidevYamlLineParseFailed', { line: line.trim() }))
    }
...
function assertTagConstraints(tags) {
  if (!tags.length) throw new Error(tx('aidevYamlTagsEmpty'))
  if (tags.length > TAGS_MAX_COUNT) {
    throw new Error(tx('aidevYamlTagsTooMany', { max: TAGS_MAX_COUNT }))
  }
  for (const tag of tags) {
    if (tag.length > TAG_MAX_LENGTH) {
      throw new Error(tx('aidevYamlTagTooLong', { max: TAG_MAX_LENGTH, tag }))
    }
  }
}
...
function parseAidevpushYaml(text) {
  ...
  for (const key of Object.keys(root)) {
    if (FORBIDDEN_TOP_KEYS.has(key)) {
      throw new Error(tx('aidevYamlForbiddenKey', { key }))
    }
  }

  const version = root.version
  if (version !== undefined && version !== 1 && version !== '1') {
    throw new Error(tx('aidevYamlVersionInvalid'))
  }

  const service_id = String(root.service_id ?? '').trim()
  if (!SERVICE_ID_RE.test(service_id)) {
    throw new Error(tx('aidevYamlServiceIdInvalid'))
  }
...
function formatAidevResolveStatus(matches) {
  ...
  if (!n) return tx('aidevMetaNoProjectMatched')
  const wsCount = uniqueWorkspaceIdsFromMatches(matches).length
  if (wsCount <= 1) return tx('aidevMetaMatchedProjects', { n })
  return tx('aidevMetaMatchedProjectsWithWs', { n, wsCount })
}
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    FORBIDDEN_TOP_KEYS, SERVICE_ID_RE, TAG_MAX_LENGTH, TAGS_MAX_COUNT,
    stripYamlComment, parseYamlScalar, parseSimpleYaml, normalizeTags,
    parseTagsList, assertTagConstraints, parseAidevpushYaml,
    readAidevMetaFromDocument, uniqueWorkspaceIdsFromMatches,
    projectIdsForWorkspace, formatAidevResolveStatus,
    readAidevMetaEvalExpression, readAidevMetaFromInspectedWindow,
  });
}
}
