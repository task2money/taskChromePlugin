...
/**
 * 取词延后到调用时：模块加载早于 locale hydrate，模块级字符串常量会冻结在 zh-CN（ADR-0089）。
 */
function membershipMismatchHint() {
  return tx('workspaceMembershipMismatch');
}
 ...
 * 其它租户仍返回。调用方在 deniedCount>0 且 merged 为空时抛 membershipMismatchHint()。
 */
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    uniqueCompanies, workspaceIdOf, workspaceDisplayName,
    unwrapWorkspaceRows, appendWorkspaceRows, uniqueCompanyIdCount,
    hasDuplicateDisplayNames, shouldQualifyWithCompany,
    workspaceOptionLabels, TENANT_MEMBERSHIP_DENIED_RE,
    membershipMismatchHint, isTenantMembershipDenied,
    loadWorkspacesAcrossCompanies,
  });
}
}
