...
(function (global) {
  ...
  const STEPS_EN = {
    ...
    'page-optimization-suggest': [
      ...
      'Popup Prompt skills: click Settings. The list is grouped by Accessibility / Conversion / Performance / SEO / Custom. Five platform presets are preloaded even while signed out (read-only; pick one or “Do not apply”). Titles on their own line with Manage opening the workspace prompt-skills page; sync/edit/delete below. Catalog (system) skills are read-only for tenants—no Delete, body locked. The footer “Signed out: skills stay on this device” means this popup is signed out (website login is a different session)—use Login at the top. After sign-in the popup overlays live catalog bodies for the same ids and can sync a workspace. If the service worker is briefly unreachable, a stored token still counts as signed in. Appended only on direct agent calls. API keys stay on this device.',
