...
(function (global) {
  ...
  const STEPS_EN = {
    ...
    'page-optimization-suggest': [
      'Sign in and pick workspace/project in the float panel (those last choices apply when the panel is closed). The popup Auto-innovate agent heading shows Alt+Shift+Z. Then Alt+Z (rebind in chrome://extensions/shortcuts).',
      'Under Auto-innovate agent, click Settings then enter OpenAI-compatible Base URL, model, and API Key (the panel closes after save). When all three are set the extension calls the agent directly; otherwise it uses the cloud path with no platform deduction. The key stays on this device.',
      'Popup Prompt skills (tendencies): click Settings. Saved list includes a “Do not apply a skill” radio; each title is on its own line with sync/edit/delete below; Delete asks for confirmation. Each skill can stay on this device or sync to a chosen workspace (members can see and edit). Appended only on direct agent calls; the JSON contract cannot be replaced. API keys stay on this device. Cloud fallback and create-task do not receive skills.',
    ...
    'popup-extras': [
      'Tracking toggle keeps requests across refresh. Float ball toggle at top. Click ! next to a section title for help. Auto-innovate agent heading shows Alt+Shift+Z; click Settings to save a local API key. Under Prompt skills click Settings: “Do not apply a skill” radio, New, per-row Edit/Delete (Delete confirms). Panel × does not hide ball.',
  ...
  function keyboardShortcutStepsEn(combo) {
    ...
    return [
      ...
      'Alt+Z: page optimization suggestions (pick workspace in the float panel).',
