...
(function (global) {
  ...
  const STEPS_EN = {
    ...
    'page-optimization-suggest': [
      ...
      'Popup Prompt skills (tendencies): after login they sync to your account (switch computers to pull). Appended only on direct agent calls; the JSON contract cannot be replaced. API keys stay on this device. Cloud fallback and create-task do not receive skills.',
    ...
    'popup-extras': [
      'Tracking toggle keeps requests across refresh. Float ball toggle at top. Click ! next to a section title for help. Click Settings under Auto-innovate agent to save a local API key. Manage local prompt skills (tendencies) for direct Alt+Z only. Panel × does not hide ball.',
