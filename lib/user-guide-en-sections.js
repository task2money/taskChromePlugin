...
(function (global) {
  ...
  const STEPS_EN = {
    overview: [
      ...
      'The popup and DevTools panel headers show the installed version. Unpacked developer-mode installs are marked Beta and checked against the GitHub Release; if a newer build exists, the header links to that zip.',
    ],
    login: [
    ...
    'page-optimization-suggest': [
      'Choose the call path under Auto-innovate agent: Direct with my key (free; Settings for an OpenAI-compatible Base URL, model, and API Key; works while signed out) or Platform backend (sign-in required). If you have not chosen, direct is the default. Direct mode can store several key groups on this device and switch the active one from the dropdown. Keys stay on this device. Popup heading shows Alt+Shift+Z.',
      'Popup Prompt skills: click Settings. Saved skills: choose a category first (Accessibility / Conversion / Performance / SEO / Custom), then pick a skill in that category. Use New to add your own skill — there is no system-vs-custom step. Only the auto-innovate skill is preloaded even while signed out (read-only, selected by default; or choose “Do not apply”). Titles on their own line with Manage opening the workspace prompt-skills page; sync/edit/delete below. Catalog (system) skills are read-only for tenants—no Delete, body locked. The footer “Signed out: skills stay on this device” means this popup is signed out (website login is a different session)—use Login at the top. After sign-in the popup overlays the live catalog body for the same id and can sync a workspace. If the service worker is briefly unreachable, a stored token still counts as signed in. Appended only on direct agent calls. API keys stay on this device.',
      ...
      'Fill all/one into float description (append only); drag the suggestion-bar handle so it does not cover the page bottom; double-click resets it to bottom center; float opens only after fill buttons; no auto task creation; cancel preview undoes changes.',
    ...
    'popup-extras': [
      'Tracking toggle keeps requests across refresh. Float ball toggle at top. Click ! next to a section title for help. Auto-innovate agent heading shows Alt+Shift+Z; in direct mode the dropdown switches saved on-device key groups, and Settings adds or edits one. Under Prompt skills click Settings: “Do not apply a skill” radio, title-row Manage opens the workspace prompt-skills page, New, custom-row Edit/Delete (Delete confirms; catalog skills read-only, no Delete). Panel × does not hide ball.',
