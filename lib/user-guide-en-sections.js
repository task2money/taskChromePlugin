...
(function (global) {
  ...
  const STEPS_EN = {
    ...
    'page-optimization-suggest': [
      'Fill a local OpenAI-compatible Base URL, model, and API Key under Auto-innovate agent → Settings. Then Alt+Z / Alt+Shift+Z work while signed out (direct agent). Without a key, sign in for the cloud path (no platform deduction). The key stays on this device. Popup heading shows Alt+Shift+Z.',
      'Popup Prompt skills: click Settings. List includes “Do not apply a skill”; titles on their own line with Manage opening the workspace prompt-skills page; sync/edit/delete below. The footer “Signed out: skills stay on this device” means this popup is signed out (website login is a different session)—use Login at the top. After sign-in the popup loads the admin “new workspace default” skill if you have not chosen one yet, and can sync a workspace. If the service worker is briefly unreachable, a stored token still counts as signed in. If the summary still shows unused and the list is empty, close and reopen the popup. Appended only on direct agent calls. API keys stay on this device.',
    ...
    'popup-extras': [
      'Tracking toggle keeps requests across refresh. Float ball toggle at top. Click ! next to a section title for help. Auto-innovate agent heading shows Alt+Shift+Z; click Settings to save a local API key. Under Prompt skills click Settings: “Do not apply a skill” radio, title-row Manage opens the workspace prompt-skills page, New, per-row Edit/Delete (Delete confirms). Panel × does not hide ball.',
