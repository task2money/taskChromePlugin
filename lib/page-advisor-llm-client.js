...
const PageAdvisorLLM = (() => {
  ...
  // redactSnippet 截断 + 抹掉本机 Key，供本地 console.warn 与观测上报共用
  // （OPT-20260922-035：上报到服务端的片段必须同样是脱敏后的）。
  function redactSnippet(content, apiKey, maxChars) {
    const limit = Number.isFinite(maxChars) && maxChars > 0 ? maxChars : MAX_LLM_SNIPPET_LOG_CHARS;
    let s = String(content || '').trim();
    // 先抹密钥再截断：否则替换后的占位串会把长度顶回上限之上。
  ...
  function logSnippet(content, apiKey) {
    return redactSnippet(content, apiKey);
  }
  ...
  // locale=en 时用英译系统提示替换中文 SYSTEM_PROMPT（OPT-20260922-038），
  // 避免系统提示语言与目标回文不一致导致建议卡夹杂中文。
  function systemContent(locale) {
    if (typeof PageAdvisorLocalePrompt !== 'undefined'
      && typeof PageAdvisorLocalePrompt.systemPromptOverride === 'function') {
      return PageAdvisorLocalePrompt.systemPromptOverride(locale) || SYSTEM_PROMPT;
    }
    return SYSTEM_PROMPT;
  }

  function buildChatMessages(page, skill, locale) {
    const messages = [{ role: 'system', content: systemContent(locale || page?.locale) }];
  ...
  async function suggest(creds, page, opts = {}) {
    ...
    if (!res.ok) {
      const err = new Error(`llm http ${res.status}: ${String(text).slice(0, 240)}`);
      // snippet 在 Key 作用域内就地脱敏，供观测上报复用（OPT-20260922-035）。
      err.status = res.status;
      err.snippet = redactSnippet(text, apiKey);
      throw err;
    }
