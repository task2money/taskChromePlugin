...
(function initCreateTaskGitIdentity(global) {
  ...
  function validate(autoRun, selections, requiredRepoUrls) {
    ...
    for (const url of required) {
      ...
      if (!row || !row.git_identity_id) {
        return tx('gitIdentityRequiredForRepo', { url });
      }
    }
    return '';
  }
  ...
  function buildSelectHtml({
    ...
    html += `<label style="display:block;font-size:10px;color:#a6adc8;margin-bottom:2px">${tx('gitIdentityLabel')}</label>`;
    ...
    html += `<option value="">${tx('gitIdentitySelectPlaceholder')}</option>`;
    const rows = Array.isArray(identities) ? identities : [];
    ...
    if (href) {
      ...
      html += ` style="display:inline-block;font-size:10px;margin-top:2px;color:#89b4fa">${tx('gitIdentityManageLink')}</a>`;
    }
    html += '</div>';
    return html;
  }
