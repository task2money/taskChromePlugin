...
function findElementByAnchorText(root, anchorText) {
  ...
  const stamped = root.querySelectorAll(`[${NID_ATTR}]`);
  for (const el of stamped) {
    if (matchText(el)) return el;
  }
  const fallback = root.querySelectorAll('button, a, [role="button"], input, textarea, label');
  for (const el of fallback) {
    if (typeof el.hasAttribute === 'function' && el.hasAttribute(NID_ATTR)) continue;
    if (matchText(el)) return el;
  }
  return null;
}
...
function resolveLiveNid(root, resolveNid, nid) {
  ...
  if (root && typeof root.querySelector === 'function') {
    try {
      const found = root.querySelector(`[${NID_ATTR}="${id}"]`);
...
function rebindSuggestionPreview(session, suggestion, deps) {
  ...
  if (!rebound) {
    ...
    if (rebound && targetNid && typeof rebound.setAttribute === 'function') {
      rebound.setAttribute(NID_ATTR, targetNid.replace(/"/g, ''));
    }
  }
  ...
  const walk = (node) => {
    ...
    for (const child of kids) {
      ...
      for (const attr of Array.from(child.attributes || [])) {
        ...
        if (name.startsWith('on') || name === 'href' && String(attr.value || '').toLowerCase().startsWith('javascript:')) {
