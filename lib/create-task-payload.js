...
function shortenRepoLabel(url, index) {
  ...
  const label = tx('taskRepoLabel', { index: index + 1 });
  if (!raw) return label;
  try {
    const u = new URL(raw.replace(/^git@([^:]+):/, 'https://$1/'));
    const path = (u.pathname || '').replace(/\.git$/, '').replace(/^\//, '');
    return tx('taskRepoLabelWithPath', { label, path: path || u.hostname });
  } catch (_) {
    return tx('taskRepoLabelWithPath', {
      label,
      path: raw.length > 48 ? `${raw.slice(0, 45)}…` : raw,
    });
  }
}
...
function validateCreateTaskForm(form = {}) {
  ...
  if (!title) return tx('validateTaskTitleRequired');
  const workspaceId = String(form.workspaceId || form.workspace_id || '').trim();
  if (!workspaceId) return tx('validateWorkspaceRequired');
  const owner = String(form.owner || '').trim();
  if (!owner) return tx('validateOwnerRequired');
  const projectIds = Array.isArray(form.projectIds)
    ? form.projectIds
    : (Array.isArray(form.projects) ? form.projects : []);
  if (!projectIds.length) return tx('validateProjectRequired');

  const containerImageId = String(form.container_image_id || form.containerImageId || '').trim();
  const hasContainerImage = Boolean(containerImageId);
  const source = normalizeFeatureParamsSource(form.feature_params_source);
  // 与 work-panel 对齐：未绑定镜像时智能体资源配置可选；个人半填仍阻断。
  if (hasContainerImage) {
    if (!source) return tx('validateFeatureParamsRequired');
    if (source === 'personal' && !String(form.personal_feature_params_config_id || '').trim()) {
      return tx('validateFeatureParamsRequired');
    }
  } else if (source === 'personal' && !String(form.personal_feature_params_config_id || '').trim()) {
    return tx('validateFeatureParamsRequired');
  }
 ...
 * 以函数而非模块级常量暴露：模块加载早于 locale hydrate，
 * 模块级字符串常量会在 zh-CN 上冻结，切 en 后仍是中文（ADR-0089）。
 */
function repoBaseLabel() { return tx('panelBaseBranch'); }
function repoBaseHint() { return tx('panelBaseBranchPlaceholder'); }
function repoBaseEmptyHint() { return repoBaseHint(); }
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    FEATURE_PARAMS_SOURCES, pad2, formatDateTimeLocal,
    getDefaultTaskDeadline, normalizePriority, normalizeFeatureParamsSource,
    buildFeatureParamsFields, repoBaseBranchKey, normalizeAssignees,
    shortenRepoLabel, RepoBaseBranchDatalist, resolveBaseForRepo,
    buildProjectsFromSelection, gitIdentityModule, autoRunLabelModule,
    validateCreateTaskForm, buildCreateTaskPayload, escapeAttr,
    repoBaseLabel, repoBaseHint, repoBaseEmptyHint,
    buildRepoBaseEditorsHtml, shouldEnableDescReset,
    readRepoBaseBranchesFromRoot,
  });
}
}
