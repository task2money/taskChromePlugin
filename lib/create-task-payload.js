...
function buildRepoBaseEditorsHtml({
  ...
  for (const projectId of ids) {
    ...
    rows.forEach((url, i) => {
      ...
      const GitId = gitIdentityModule();
      if (GitId && gitIdentity && gitIdentity.enabled && String(url).trim()) {
        html += GitId.buildSelectHtml({
          repoUrl: String(url).trim(),
          identities: gitIdentity.identities || [],
          previousByUrl: gitIdentity.previousByUrl || {},
          settingsHref: gitIdentity.settingsHref || '',
        });
      }
      html += `</div>`;
    });
