...
const API = (() => {
  ...
  const DEFAULT_ENDPOINTS = {
    ...
    userProfile: '/api/accounts/users/profile/',
  ...
  async function resolveCompanyId(workspaceId) {
    const wid = String(workspaceId || '').trim();
    if (!wid) {
      throw new Error(tx('swMissingWorkspaceId'));
    }
    ...
    throw new Error(tx('apiResolveTenantFailed', { wid }));
  }
  ...
  async function fetchCurrentUser() {
    if (!userId) {
      throw new Error(tx('apiMissingUserId'));
    }
    return request('GET', `/api/user/${encodeURIComponent(userId)}/accounts/users/me/`);
  }

  /** GET auth_user_profile including preferred_locale (ADR-0089). */
  async function getUserProfile() {
    return request('GET', endpoints.userProfile || '/api/accounts/users/profile/');
  }

  /** PATCH auth_user_profile (preferred_locale etc). */
  async function patchUserProfile(body) {
    return request('PATCH', endpoints.userProfile || '/api/accounts/users/profile/', body || {});
  }
  ...
  async function getWorkspaces(companyId) {
    ...
    if (!merged.length && deniedCount > 0) {
      const err = new Error(WL.membershipMismatchHint());
    ...
    try {
      for (const taskData of tasksData) {
        try {
          ...
          errors.push({ task: taskData.title || tx('taskUntitled'), error: e.message });
        }
      }
    } finally {
  ...
  async function uploadPluginScreenshot(dataUrl) {
    ...
    if (!m) throw new Error(tx('apiScreenshotDataUrlInvalid'));
  ...
  async function resolveAidevMeta({ serviceId, tag } = {}) {
    ...
    if (!sid && !tagStr) {
      throw new Error(tx('apiServiceIdOrTagRequired'));
    }
  ...
  return {
    ...
    setUserId, getUserId, fetchCurrentUser, getUserProfile, patchUserProfile,
