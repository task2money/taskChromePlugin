...
const API = (() => {
  ...
  const DEFAULT_ENDPOINTS = {
    ...
    projects:         '/api/projects/?workspace_id={workspaceId}',
    members:          '/api/tenant/{companyId}/accounts/members/',
    createTask:       '/api/projects/todos/',
    batchTasks:       '/api/projects/todos/',   // 无批量端点，逐个创建
   ...
   * 初始化 API 客户端
   */
  function init(baseUrlOverride, tokenOverride, endpointMapping) {
  ...
  async function loginWithAccessToken(accessToken) {
    ...
    const data = await request('POST', path, { access_token: accessToken });
    if (data.token || data.access_token) {
      token = data.token || data.access_token;
    }
    return data;
  ...
  async function getWorkspaces() {
    return request('GET', buildPath(endpoints.workspaces));
  }
  ...
  async function getProjects(workspaceId) {
    return request('GET', buildPath(endpoints.projects, { workspaceId }));
  }
  ...
  /**
   * 创建任务 (单个) — 自动将插件字段名映射到 API 字段名
   */
  async function createTask(taskData) {
    const apiData = { ...taskData };
    // 字段名映射: workspaceId → workspace_id
    if (apiData.workspaceId && !apiData.workspace_id) {
      apiData.workspace_id = apiData.workspaceId;
      delete apiData.workspaceId;
    }
    // 字段名映射: projectIds → projects ([{id, base_branch}])
    if (Array.isArray(apiData.projectIds) && !apiData.projects) {
      apiData.projects = apiData.projectIds.map(project_id => ({ project_id, base_branch: 'main' }));
      delete apiData.projectIds;
    }
    // 规范化 projects 数组：确保每项都有 base_branch
    if (Array.isArray(apiData.projects)) {
      apiData.projects = apiData.projects.map(p =>
        typeof p === 'object' ? { ...p, base_branch: p.base_branch || 'main' } : p
      );
    }
    // priority 字符串 → 数字 (low=0, medium=1, high=2, critical=3)
    if (typeof apiData.priority === 'string') {
      const PRIORITY_MAP = { low: 0, medium: 1, high: 2, critical: 3 };
      apiData.priority = PRIORITY_MAP[apiData.priority] ?? 1;
    }
    // 注入 owner (从配置读取)
    if (endpointOwner && !apiData.owner) {
      apiData.owner = endpointOwner;
    }
    return request('POST', buildPath(endpoints.createTask), apiData);
  }
  ...
  return {
    ...
    request, login, loginWithAccessToken,
    getWorkspaces, getProjects, getMembers, fetchProgressColumns, getBranches, createTask, createTasksBatch,
    getToken, getBaseUrl,
