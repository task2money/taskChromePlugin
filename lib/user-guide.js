...
(function initUserGuide(global) {
  ...
  // 快捷键组合串状态：''（未知，展示默认 Ctrl+Shift+X 文案）| 'Ctrl+Shift+X' 等规范组合
  let shortcutCombo = '';

  /**
   * 快捷键步骤按用户当前自定义的组合动态插值（OPT-20260806-017 演进）：
   * 未设置/未知时展示默认 Ctrl+Shift+X；已自定义时展示实际组合，
   * 与浏览器级键位（chrome.commands.update）及页内兜底监听的严格匹配保持一致。
   */
  function keyboardShortcutSteps() {
    const combo = shortcutCombo || 'Ctrl+Shift+X';
    return [
      ...
      '快捷键无效？插件内置页内兜底监听，浏览器级注册失败时在页面内按所选组合仍可用；也可到 chrome://extensions/shortcuts 检查键位绑定（默认 Ctrl+Shift+X，可能被其他扩展占用）。',
    ];
  }
