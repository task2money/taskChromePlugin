...
(function initUserGuide(global) {
  ...
  // 快捷键模式状态：''（未知，展示通用文案）| 'cmd' | 'ctrl'
  let shortcutMode = '';

  /**
   * 快捷键步骤按用户当前选择的键位动态插值（OPT-20260806-017）：
   * 未选择（默认按系统）展示通用文案；已在弹窗选择 ⌘/Ctrl 时展示实际组合，
   * 与页内兜底监听的严格匹配保持一致，避免使用说明与实际行为不一致。
   */
  function keyboardShortcutSteps() {
    const combo = shortcutMode === 'cmd' ? '⌘+Shift+X' : shortcutMode === 'ctrl' ? 'Ctrl+Shift+X' : '⌘/Ctrl+Shift+X';
    const comboNote = shortcutMode ? '' : '（默认按系统：Mac ⌘、其他系统 Ctrl）';
    return [
      `${combo}${comboNote}：切换指针选择模式，在任意页面选取元素加入任务描述。`,
      '在扩展弹窗「快捷键」中可自由切换唤起组合为 ⌘+Shift+X 或 Ctrl+Shift+X；页内兜底监听与说明跟随该选择。',
      ...
      '快捷键无效？插件内置页内兜底监听，浏览器级注册失败时在页面内按所选组合仍可用；也可到 chrome://extensions/shortcuts 检查键位绑定（浏览器级键位按系统预设：Mac ⌘、其他 Ctrl）。',
    ];
  }

  function setShortcutMode(mode) {
    // 仅接受 'cmd' / 'ctrl'；其他值（含空）回退通用文案
    shortcutMode = mode === 'cmd' || mode === 'ctrl' ? mode : '';
  }

  /** 从 storage 加载用户选择的快捷键模式（渲染前调用；无 chrome API 环境安全返回 ''） */
