...
(function initUserGuide(global) {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      title: '区域框选后自动创新（Alt+Shift+E）',
      surfaces: ['float', 'popup', 'panel'],
      steps: [
        '登录并选好工作空间后，按 Alt+Shift+E（可在 chrome://extensions/shortcuts 改绑）进入框选模式；进入时会暂时收起页内浮窗面板，避免挡住宿主页。',
        '在宿主页面上拖拽画出矩形区域（框选层盖在页面之上，不采集浮窗自身文案）；松手后仅对该区域内的可见文案与 DOM 大纲生成优化建议（流程与 Alt+E 相同）。',
        '选区过小会提示重新框选；按 Esc 取消框选。再按一次 Alt+Shift+E 也可退出框选模式。',
  ...
  function keyboardShortcutSteps() {
    ...
    return [
      ...
      'Alt+Shift+E（默认，可在 chrome://extensions/shortcuts 改绑）：先框选页面区域，再仅对该区域生成优化建议（后续交互与 Alt+E 相同）。',
      ...
      'Esc：取消指针选择或区域框选；多选状态下先清空多选，再按退出指针模式；关闭调整弹窗。',
