  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      steps: [
        ...
        '「是否自动运行」随所选项目是否允许自动运行而启用或禁用：不允许或未选项目时不可勾选；允许时默认勾选，可再取消。勾选后须为关联仓库选择 Git 提交身份。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择工作空间、项目（单选）、进度列、优先级、负责人 → 填写分支策略与任务信息 →「创建任务」。项目名称旁标注「可自动运行」或「不可自动运行」。「是否自动运行」随所选项目是否允许自动运行而启用或禁用。勾选自动运行时须设置 Git 提交身份。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择批量创建目标（工作空间、单选项目、进度、交付物、镜像、环境变量、截止日期、自动运行、Git 提交身份、分支等）→「批量创建任务」。项目列表在名称旁标注「可自动运行」或「不可自动运行」；自动运行随所选项目是否允许而启用或禁用。',
