...
(function initUserGuide(global) {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      surfaces: ['float', 'popup', 'panel'],
      steps: [
        '先登录并在浮窗选择工作空间，再按 Alt+E（可在 chrome://extensions/shortcuts 改绑）。',
  ...
  function keyboardShortcutSteps() {
    ...
    return [
      ...
      'Alt+E（默认，可在 chrome://extensions/shortcuts 改绑）：采集当前页并生成优化建议列表；多选后填入浮窗任务描述，不自动创建任务。使用前须登录并在浮窗选择工作空间。',
