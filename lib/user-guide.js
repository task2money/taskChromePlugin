/**
 * 云端 Coding — 面向用户的使用说明（UI 渲染 SSOT）
 *
...
(function initUserGuide(global) {
  ...
  const SECTIONS = [
    {
      ...
      steps: [
        ...
        '主要入口：① 扩展弹窗登录 ② 页内悬浮球快速建任务 ③ DevTools「云端 Coding」面板（单请求 / 批量错误）。',
      ],
    },
    {
      id: 'login',
      ...
      steps: [
        ...
        '登录与快捷键变更通过 storage 同步到各页，不向其它标签页广播。',
      ],
    },
    {
      id: 'float-create',
      ...
      steps: [
        ...
        '选择工作空间与项目（可多选）→ 填写标题、描述、优先级、进度、交付物、镜像、智能体资源配置（必填）、截止日期等。若同名工作空间来自不同公司，选项会显示「名称 · 公司名」。',
    ...
    },
    {
      ...
      steps: [
        '按 F12 打开开发者工具 → 切换到「云端 Coding」面板 →「单请求创建」Tab。',
        ...
        '选择工作空间、项目、进度列、优先级、负责人 → 填写分支策略与任务信息 →「创建任务」。勾选自动运行时须设置 Git 提交身份。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择批量创建目标（工作空间、项目、进度、交付物、镜像、环境变量、截止日期、自动运行、Git 提交身份、分支等）→「批量创建任务」。',
    ...
    },
    {
      ...
      steps: [
        ...
        '请求预览可展开查看近期捕获；详细建任务请用 DevTools 云端 Coding 或页内浮窗。',
        '各标签页的任务描述互相独立，不会同步到其他页面。',
  ...
  function keyboardShortcutSteps() {
    ...
    return [
      ...
      'F12：打开 Chrome DevTools → 切换到 云端 Coding 面板创建任务或批量捕获错误。',
  ...
  function renderFullGuideHtml(opts) {
    ...
    const title = opts?.title || '云端 Coding 使用说明';
    const body = getSectionsForSurface(surface).map(renderSectionHtml).join('');
    return (
