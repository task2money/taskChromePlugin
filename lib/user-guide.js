...
(function initUserGuide(global) {
  ...
  function renderCollapsibleHtml(opts) {
    ...
    return (
      ...
      + `<button type="button" class="tcp-guide-root-summary" aria-expanded="${expanded ? 'true' : 'false'}" aria-controls="${panelId}" data-guide-toggle>`
      + `${escapeHtml(title)}`
      + `</button>`
      + `<div class="tcp-guide-body" id="${panelId}"${expanded ? '' : ' hidden'}>`
