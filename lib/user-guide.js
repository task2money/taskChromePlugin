  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      steps: [
        ...
        '选择工作空间与项目（可多选）→ 填写标题、描述、优先级、进度、交付物、镜像、智能体资源配置（必填）、截止日期等。若同名工作空间来自不同公司，选项会显示「名称 · 公司名」。每个项目名称旁标注「可自动运行」或「不可自动运行」（与项目设置「是否允许自动运行」一致）。',
        '可勾选「是否自动运行」。勾选后须为每个关联仓库选择 Git 提交身份（未勾选则不显示、不要求该字段）。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择工作空间、项目、进度列、优先级、负责人 → 填写分支策略与任务信息 →「创建任务」。项目名称旁标注「可自动运行」或「不可自动运行」。勾选自动运行时须设置 Git 提交身份。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择批量创建目标（工作空间、项目、进度、交付物、镜像、环境变量、截止日期、自动运行、Git 提交身份、分支等）→「批量创建任务」。项目列表同样在名称旁标注「可自动运行」或「不可自动运行」。',
