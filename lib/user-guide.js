...
(function initUserGuide(global) {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      steps: [
        ...
        '插件采集当前页可见文本并请求优化建议；生成中会显示加载提示。',
        '在建议列表中多选后点「填入任务描述」：按勾选顺序追加结构化区块到浮窗描述，不会自动创建任务。',
  ...
  function keyboardShortcutSteps() {
    ...
    return [
      ...
      'Alt+E（默认，可在 chrome://extensions/shortcuts 改绑）：采集当前页并生成优化建议列表；多选后填入浮窗任务描述，不自动创建任务。使用前须登录；请在扩展弹窗「Alt+E 默认上下文」或浮窗中选择工作空间。',
