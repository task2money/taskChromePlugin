  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      steps: [
        ...
        '普通点击：选中单个元素 → 填写调整期望 → 确认后追加到任务描述（含页面链接、选择器、可见文本）。',
        '列表 / 兄弟节点：Shift+点击起点 → 再 Shift+点击同父下的相邻兄弟，一次选中连续区间；锚定后按住 Shift 悬停可预览高亮。',
        '支持 Shadow DOM（选择器含 >>>）、同源/跨域 iframe、原生控件 UA Shadow（内部不可穿透时选中宿主）。',
        '可选「附带元素截图」：裁剪后上传，描述写入 https URL（多选时截取区间并集）。',
      ],
    },
    {
      id: 'devtools-single',
