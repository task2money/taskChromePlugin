...
(function initUserGuide(global) {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      steps: [
        ...
        '「是否自动运行」随所选项目是否允许自动运行而启用或禁用：不允许或未选项目时不可勾选。项目允许时须先选择已安装镜像才能勾选（镜像标签显示「*自动运行必选」）；选好镜像后默认勾选，可再取消。不自动运行时镜像可选。勾选后须为关联仓库选择 Git 提交身份。',
        '填写逐仓基准分支、协作人员、工作分支与合并目标分支（支持模板 / 手写）。',
    ...
    },
    {
      ...
      steps: [
        '按 F12 打开开发者工具 → 切换到「云端Coding: 自动创新助手」面板 →「单请求创建」Tab。',
        '确保 Network 面板已打开并产生请求；在本面板请求列表中搜索/筛选后点选一条。点选后任务描述会自动填入方法、URL、状态、请求头/请求体、响应头/响应体。',
        '新请求实时自动出现；若列表与 Network 面板不同步，点「🔄 刷新列表」会从后台重新拉取最新请求。',
        '需要重新筛选时，可点「🗑️ 清空列表」清空本面板与后台缓存的请求（不影响 Chrome Network 面板）；之后仅显示新产生的请求。',
        '选择工作空间、项目（单选）、进度列、优先级、负责人 → 填写分支策略与任务信息 →「创建任务」。项目名称旁标注「可自动运行」或「不可自动运行」。「是否自动运行」随所选项目是否允许自动运行而启用或禁用；允许时须先选已安装镜像才能勾选。勾选自动运行时须设置 Git 提交身份。',
        '描述可用「指针选择」从页面追加元素调整块；智能体资源配置为必填。',
    ...
    },
    {
      ...
      steps: [
        ...
        '选择批量创建目标（工作空间、单选项目、进度、交付物、镜像、环境变量、截止日期、自动运行、Git 提交身份、分支等）→「批量创建任务」。项目列表在名称旁标注「可自动运行」或「不可自动运行」；自动运行随所选项目是否允许而启用或禁用，允许时须先选已安装镜像。',
    ...
    },
    {
      ...
      steps: [
        ...
        '请求预览可展开查看近期捕获；详细建任务请用 DevTools 云端Coding: 自动创新助手 或页内浮窗。',
        '各标签页的任务描述互相独立，不会同步到其他页面。',
  ...
  function keyboardShortcutSteps() {
    ...
    return [
      ...
      'F12：打开 Chrome DevTools → 切换到 云端Coding: 自动创新助手 面板创建任务或批量捕获错误。',
  ...
  function renderFullGuideHtml(opts) {
    ...
    const title = opts?.title || '云端Coding: 自动创新助手 使用说明';
    const body = getSectionsForSurface(surface).map(renderSectionHtml).join('');
    return (
