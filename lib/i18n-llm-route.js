...
(function (global) {
  const zh = {
    paLlmSectionHint: '在本区选择调用方式。直连我的 Key 为免费；调用平台后端会消耗资源。未选择时默认直连。密钥只保存在本机。快捷键见标题旁 Alt+Shift+Z，整页快捷键走同一调用方式。点击「设置」填写 API Key。',
    paLlmRouteLegend: '调用方式',
    paLlmRouteDirect: '直连我的 Key（免费）',
    paLlmRouteSaas: '调用平台后端（消耗资源）',
  ...
  const en = {
    paLlmSectionHint: 'Choose the call path in this section. Direct with my key is free. The platform backend uses resources. Direct is the default until you choose. The key stays on this device. See Alt+Shift+Z in the heading; the whole-page shortcut uses the same path. Click Settings to enter the API Key.',
    paLlmRouteLegend: 'Call path',
    paLlmRouteDirect: 'Direct with my key (free)',
    paLlmRouteSaas: 'Platform backend (uses resources)',
