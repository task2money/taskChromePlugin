...
if (typeof require === 'function' && typeof globalThis !== 'undefined' && !globalThis.PageAdvisorPromptSkillCatalog) {
  require('./page-advisor-prompt-skill-catalog.js');
}
...
const PageAdvisorPromptSkills = (() => {
  const STORAGE_KEYS = {
    ...
    // 类别来源选择（system/custom）须随本机文档落盘，否则刷新后丢失并导致下次 PUT 漏发
    // category_choices（OPT-20260922-043）
    categoryChoices: 'pageAdvisorCategoryChoices',
  };
  const MAX_SKILLS = 20;
  const MAX_BODY = 8000;
  const MAX_TITLE = 80;
  ...
  async function loadFromStorage(storageApi) {
    ...
    const raw = await store.get([STORAGE_KEYS.list, STORAGE_KEYS.activeId, STORAGE_KEYS.categoryChoices]);
    return normalizeStore({
      skills: raw?.[STORAGE_KEYS.list],
      activeSkillId: raw?.[STORAGE_KEYS.activeId],
      categoryChoices: raw?.[STORAGE_KEYS.categoryChoices],
    });
  }
  ...
  async function saveToStorage(storeObj, storageApi) {
    ...
    return api.set({
      ...
      [STORAGE_KEYS.categoryChoices]: st.categoryChoices,
    });
  }
  ...
  /** 预埋仅自动创新一条；已显式「不应用」不改 active；退役预埋 id 卸下。 */
  function applySystemCatalogDefault(localStore, catalog, syncTarget, opts) {
    const Cat = typeof globalThis !== 'undefined' ? globalThis.PageAdvisorPromptSkillCatalog : null;
    if (!Cat) return { store: normalizeStore(localStore), action: 'noop' };
    return Cat.applySystemCatalogDefault(localStore, catalog, syncTarget, opts, catalogApi());
  }

  function systemSkillForTendency(storeObj, tendency) {
    const Cat = typeof globalThis !== 'undefined' ? globalThis.PageAdvisorPromptSkillCatalog : null;
    if (!Cat) return null;
    return Cat.systemSkillForTendency(storeObj, tendency, catalogApi());
  }

  function categorySourceOf(storeObj, tendency) {
    const Cat = typeof globalThis !== 'undefined' ? globalThis.PageAdvisorPromptSkillCatalog : null;
    if (!Cat) return 'system';
    return Cat.categorySourceOf(storeObj, tendency, catalogApi());
  }

  function selectCategorySource(storeObj, tendency, source, now = Date.now()) {
    const Cat = typeof globalThis !== 'undefined' ? globalThis.PageAdvisorPromptSkillCatalog : null;
    if (!Cat) return { store: normalizeStore(storeObj), customSkillId: '', created: false };
    return Cat.selectCategorySource(storeObj, tendency, source, now, catalogApi());
  }
  ...
  function mergeWorkspaceBundle(localStore, remotePayload, workspaceId, legacyWorkspaceId) {
    ...
    // 服务端未回传类别选择时保留本机选择（OPT-20260922-043）：客户端刚设置的
    // category_choices 不能因为一次 pull 就被清空；服务端有值则以服务端为准。
    const categoryChoices = remote.categoryChoices.length
      ? remote.categoryChoices
      : local.categoryChoices;
    return { store: { skills, activeSkillId, categoryChoices }, action: 'pull', revision };
  }
