...
const PageAdvisorPromptSkills = (() => {
  ...
  /** 预埋五类稳定 id；已有 id 不改 active；overlayExisting 时覆盖目录同 id 正文。 */
  function applySystemCatalogDefault(localStore, catalog, syncTarget, opts) {
    ...
    const overlayExisting = Boolean(opts && opts.overlayExisting) && catalogList.length > 0;
    ...
    const activeSkillId = local.activeSkillId ? local.activeSkillId : insertedCustomId;
    return {
