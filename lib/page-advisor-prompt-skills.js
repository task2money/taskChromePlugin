...
const PageAdvisorPromptSkills = (() => {
  ...
  /** 某预设类别的系统（只读目录）Skill，缺省 null。 */
  function systemSkillForTendency(storeObj, tendency) {
    const st = normalizeStore(storeObj);
    const t = normalizeTendency(tendency);
    if (!t) return null;
    return st.skills.find((s) => normalizeTendency(s.tendency) === t && isSystemPromptSkill(s)) || null;
  }

  /** 某预设类别当前选用「系统默认」还是「自行定制」，缺省 system。 */
  ...
  /**
   * 选择某预设类别用系统稿还是自行定制（OPT-20260922-043）。
   * 定制时把目录正文 clone 成本机新 id——系统只读条禁止改写；选择记入 categoryChoices，
   * 随 PUT 作为 category_choices 上行，与 SaaS 工作空间页同一语义。
   * @returns {{store: object, customSkillId: string, created: boolean}}
   */
  function selectCategorySource(storeObj, tendency, source, now = Date.now()) {
    ...
    let created = false;
    if (wantCustom) {
      const existing = skills.find((s) => normalizeTendency(s.tendency) === t && !isSystemPromptSkill(s));
      if (existing) {
        customSkillId = String(existing.id || '');
      } else {
        const sys = skills.find((s) => normalizeTendency(s.tendency) === t && isSystemPromptSkill(s));
        if (!sys || skills.length >= MAX_SKILLS) {
          return { store: st, customSkillId: '', created: false };
        }
        const clone = normalizeSkill({
          id: newSkillId(),
          title: sys.title,
          tendency: t,
          body: sys.body,
