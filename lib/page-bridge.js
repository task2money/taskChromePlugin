...
(() => {
  ...
  function handlePageMessage(event) {
    ...
    if (msg.source !== NAMESPACE) return;

    const { action, requestId, payload } = msg;
    if (!action) return;

    // 异步处理，不阻塞 postMessage 事件循环
  ...
  window.addEventListener('message', handlePing, false);

  // ---- 初始化 ----

  announceAvailability();

  // ---- 监听 SW 广播的 accountExpired 事件并转发页面 ----
  try {
    chrome.runtime.onMessage.addListener((message) => {
      if (message?.action === 'accountExpired' && message.data?.accounts) {
        try {
          window.postMessage({
            source: NAMESPACE,
      ...
      // 返回 undefined（不处理其他 message，也不 sendResponse）
    });
  } catch (_) { /* ignore */ }
