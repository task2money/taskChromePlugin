...
(function (global) {
  const zh = {
    ...
    paSkillSectionHint: '点「设置」管理 Skill。已保存的 Skill 先选择类别（无障碍/转化/性能/SEO/自定义），再直接选择该类别里的技能。想自定义时点「新建」。未登录也会预埋自动创新这一条技能（只读，默认选中）。每条可只存本机，或同步到指定工作空间。工作空间 Skill 也可在 SaaS「工作空间管理」行「自动创新提示词管理」编辑。选中后仅在直连智能体时追加到系统提示（Alt+Z 整页与 Alt+Shift+Z 区域都走直连，云端回退不带 Skill）；API Key 仍只保存在本机。',
  };
  const en = {
    paSkillPickCategory: 'Choose a category first',
    paSkillBackToCategories: 'Back to categories',
    paSkillSectionHint: 'Click Settings to manage skills. Saved skills ask you to choose a category first (Accessibility / Conversion / Performance / SEO / Custom), then a skill in that category. Use New to add your own skill. Only the auto-innovate skill is preloaded while signed out (read-only, selected by default). Each skill can stay on this device or sync to a chosen workspace. Workspace skills can also be edited under tenant Settings → workspace management → Auto-innovate prompt skills. The selected skill is appended only on direct agent calls — both Alt+Z (whole page) and Alt+Shift+Z (picked region) are direct, and cloud fallback ignores it. API keys stay on this device.',
  };
