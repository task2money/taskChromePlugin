...
const MultiAccount = (() => {
  ...
  async function upsertSavedAccount(slot) {
    ...
    if (isNew && filtered.length >= MAX_SAVED_ACCOUNTS) {
      throw new Error(tx('swMaxAccounts', { max: MAX_SAVED_ACCOUNTS }));
    }
   ...
   * 批量删除指定账号槽（过期账号清理兜底）。
   *
   * Popup 定位单账号入口后（OPT-20260806-036），槽位由 taskFE 网页端维护；
   * 本方法供 SW 消息路径（如网页端主动清理）在写入 storage 后清理已失效槽位，
   * 避免死账号在插件侧无限累积且不可见。正式包不再做后台定时 /me/ 探测。
   * 删除活跃账号时自动切换到剩余第一个账号；全部删除后清除活跃标记。
   */
  async function pruneSavedAccounts(userIds) {
    const ids = [...new Set(
    ...
    if (ids.length === 0) {
      return { list: await listSavedAccounts(), removed: [] };
    }
    const list = await listSavedAccounts();
    const removed = list.filter((s) => ids.includes(s.userId));
    if (removed.length === 0) {
      return { list, removed };
    }
    const next = list.filter((s) => !ids.includes(s.userId));
    await writeStorage(next);

    // 若删除的是当前活跃账号，清除活跃标记并回退到剩余第一个账号
    const active = await getActiveUserId();
    if (active && !next.some((s) => s.userId === active)) {
      await clearActiveUserId();
      if (next.length > 0) {
        await setActiveUserId(next[0].userId);
      }
    }
  ...
  async function setActiveUserId(userId) {
    ...
    if (!account) throw new Error(tx('swAccountNotFound', { uid }));
