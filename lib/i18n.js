...
(function (global) {
  ...
  /**
   * ADR-0089: apply auth_user_profile.preferred_locale when set.
   * Empty / invalid → keep current client resolution. Returns applied locale or null.
   * @param {{ preferred_locale?: unknown }|null|undefined} profile
   * @returns {'zh-CN'|'en'|null}
   */
  function applyPreferredLocaleFromProfile(profile) {
    const n = normalize(profile && profile.preferred_locale)
    if (!n) return null
    return setLocale(n)
  }
  ...
  const api = {
    ...
    getMessageTables,
    normalize,
    applyPreferredLocaleFromProfile,
  }
