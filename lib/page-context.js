...
function capturePageContextInRect(opts = {}) {
  ...
  while (stack.length) {
    ...
    // 同源 iframe：区域相交时换算子文档坐标并采集内部可见文本（跨域则跳过）
    if (tag === 'IFRAME' && intersects(region, rect)) {
      let childDoc = null;
      try {
        childDoc = el.contentDocument || (el.contentWindow && el.contentWindow.document) || null;
      } catch (_) {
        childDoc = null;
      }
      if (childDoc) {
        ...
        if (childRoot) {
          const nested = capturePageContextInRect({
            ...opts,
            root: childRoot,
            url: opts.url,
            title: opts.title,
            region: {
              left: (Number(region.left) || 0) - (Number(rect.left) || 0),
              top: (Number(region.top) || 0) - (Number(rect.top) || 0),
              width: Number(region.width) || 0,
              height: Number(region.height) || 0,
            },
            getBoundingClientRect: getRect,
          });
          if (nested && nested.pageText) {
            iframeTexts.push(String(nested.pageText));
          }
        }
      }
      continue;
    ...
    if (isElementSkipped(el)) continue;
    const children = el.children || el.childNodes || [];
    let hasWalkableElementChild = false;
    for (let i = 0; i < children.length; i += 1) {
      const c = children[i];
      if (!c || c.nodeType !== 1) continue;
      const cTag = String(c.tagName || '').toUpperCase();
      if (cTag === 'IFRAME' || !isElementSkipped(c)) {
        hasWalkableElementChild = true;
        break;
      }
    }
    // 只采叶子相交节点，避免祖先容器把区域外文案一并 extract
    if (intersects(region, rect) && !hasWalkableElementChild) hits.push(el);
    for (let i = children.length - 1; i >= 0; i -= 1) {
      stack.push(children[i]);
    }
  }
...
if (typeof globalThis !== 'undefined') {
  Object.assign(globalThis, {
    MAX_PAGE_TEXT_RUNES, MAX_DOM_OUTLINE_NODES, NID_ATTR, SKIP_TAGS,
    OUTLINE_TAGS, truncatePageText, isPluginChromeUi, isElementSkipped,
    extractVisibleReadableText, capturePageContext, stampDomNids,
    captureDomOutline, clearDomNids, capturePageContextInRect,
    capturePageContextForElements,
  });
}
}
