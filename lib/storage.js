...
const Storage = {
  ...
  async getApiConfig() {
    ...
    const GATEWAY_DEFAULT = 'http://183.250.1.132:18081';
    const WRONG_FRONTEND_DEFAULT = 'http://183.250.1.132:4000';
    let baseUrl = cfg.baseUrl || GATEWAY_DEFAULT;
    // 4000 为 Vue 前端，不代理 /api；自动迁移到 taskGateway
    if (baseUrl === WRONG_FRONTEND_DEFAULT) {
  ...
  /** 保存 API 配置（含 token 过期信息） */
  async saveApiConfig(baseUrl, token, expiresIn) {
    const payload = { baseUrl, token };
    if (expiresIn > 0) {
      const now = Math.floor(Date.now() / 1000);
      payload.tokenIssuedAt = now;
      payload.tokenExpiresAt = now + expiresIn;
    } else if (token) {
      // When saving a token without explicit expiry, preserve existing expiry if set
      const existing = await this.get(['tokenExpiresAt', 'tokenIssuedAt']);
      if (existing.tokenExpiresAt) {
        payload.tokenExpiresAt = existing.tokenExpiresAt;
        payload.tokenIssuedAt = existing.tokenIssuedAt;
      }
    } else {
