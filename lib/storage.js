...
globalThis.Storage = {
  ...
  /** 获取 API 配置 */
  async getApiConfig() {
    const cfg = await this.get(['baseUrl', 'token', 'tokenExpiresAt', 'tokenIssuedAt', 'refreshToken']);
   ...
   * refreshToken（OIDC offline_access，OPT-20260824-052）：
   * - 显式传入非空字符串 → 覆盖保存（登录/刷新轮换后写入新值）
   * - 省略参数 → 保留既有 refreshToken（旧调用方三参形式不受影响）
   * - 清除 token（token 为空）→ 一并清空 refreshToken
   */
  async saveApiConfig(baseUrl, token, expiresIn, refreshToken) {
    const payload = { token };
  ...
  /** 清除登录态（token / refresh token / 凭据），保留服务器地址 */
  async clearAuth() {
    return this.set({
      token: '',
      tokenIssuedAt: 0,
  ...
  formatTokenExpiryHint(remainingSeconds) {
    ...
    return {
      text: tx('storageTokenExpiringSoon', { mins }),
  ...
  // ---- 元素拾取快捷键配置（可自定义；默认 Alt+X，全平台统一）----

  /**
   * 默认快捷键：全平台 'Alt+X'。
   * 冲突核查（2026-09-11）：Chrome 官方快捷键无 Alt+X；Tab Groups 占用的是 Alt+Shift+X；
   * Windows「最大化」为 Alt+Space 再按 X（序列，非同时）；故选用 Alt+X 作为统一默认。
   * isMacPlatform 仍用于 shortcutToPlatformBinding（Ctrl→MacCtrl）。
   */
  detectDefaultShortcut(_nav) {
    return this.SHORTCUT_DEFAULT;
  },

  /** 平台探测：macOS → true（可注入 navigator 供测试；改绑 MacCtrl 用） */
  isMacPlatform(nav) {
  ...
  /**
   * 默认快捷键常量（全平台统一 Alt+X）。
   * 组合串模型：'Alt+X' / 'Ctrl+Shift+X' / 'Alt+Z' / 'Command+Shift+X' 等，
   * 浏览器级键位经 chrome.commands.update 动态改绑，页内兜底监听跟随同一配置。
   * 注意：旧版 storage 值 'ctrl' 迁移固定为 'Ctrl+Shift+X'，不得改绑到本常量。
   */
  get SHORTCUT_DEFAULT() { return 'Alt+X'; },

  /**
   * 页面优化建议默认快捷键（与 manifest suggested_key / 页内兜底一致）。
   * 左手单手：Alt+X 选元素，Alt+Z 全页建议，Alt+Shift+Z 区域建议。
   */
  get PAGE_ADVISOR_SHORTCUT() { return 'Alt+Z'; },
  get PAGE_ADVISOR_REGION_SHORTCUT() { return 'Alt+Shift+Z'; },

  /** 允许的修饰键（对齐 chrome.commands 规范子集；Command 仅限 macOS） */
  _SHORTCUT_MODIFIERS() { return ['Ctrl', 'Alt', 'Command', 'Shift']; },

  /**
   * 允许的按键：chrome.commands 子集 + 反引号（自定义拾取组合可用）。
   * 反引号规范名为 Backquote；亦接受字面 '`'。
   */
  _SHORTCUT_KEYS() {
    return new Set([
  ...
  normalizeShortcut(raw) {
    ...
    if (key === '`' || key.toLowerCase() === 'backquote') key = 'Backquote';
  ...
  /**
   * 获取元素拾取快捷键配置 — 返回规范组合串。
   * 旧版 'cmd'/'ctrl' 模式自动迁移：cmd → Command+Shift+X，ctrl → Ctrl+Shift+X（历史值，非当前默认）；
   * 未设置 / 非法值回退统一默认 Alt+X。
   */
  ...
  async saveElementPickerShortcut(shortcut) {
    ...
    if (!normalized) throw new Error(tx('swShortcutInvalid', { shortcut: String(shortcut) }));
   ...
   * 用户自定义的 'Ctrl+...' 组合在 mac 上才会触发此转换；默认 Alt+X 无需转换。
   */
  ...
  matchShortcutKeydown(e, shortcut) {
    ...
    if (eventKey === '`' || eventKey === 'Backquote' || e.code === 'Backquote') {
      eventKey = 'Backquote';
    }
