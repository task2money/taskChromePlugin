...
globalThis.Storage = {
  ...
  /**
   * 默认快捷键常量（全平台统一 Alt+X）。
   * 组合串模型：'Alt+X' / 'Ctrl+Shift+X' / 'Alt+`' / 'Command+Shift+X' 等，
  ...
  /**
   * 页面优化建议默认快捷键（页内兜底）。
   * chrome.commands 不支持反引号键，故 manifest 不设 suggested_key，由 content 监听。
   */
  get PAGE_ADVISOR_SHORTCUT() { return 'Alt+`'; },
  get PAGE_ADVISOR_REGION_SHORTCUT() { return 'Alt+Shift+`'; },

  /** 允许的修饰键（对齐 chrome.commands 规范子集；Command 仅限 macOS） */
  _SHORTCUT_MODIFIERS() { return ['Ctrl', 'Alt', 'Command', 'Shift']; },

  /**
   * 允许的按键：chrome.commands 子集 + 反引号（页内兜底专用，commands 无法注册）。
