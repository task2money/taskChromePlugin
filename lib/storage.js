...
const Storage = {
  ...
  // ---- 元素拾取快捷键配置（可自定义，默认 Ctrl+Shift+X）----

  /**
   * 默认快捷键：统一 Ctrl+Shift+X（所有平台，含 macOS）。
   * 旧版（OPT-20260806-001~003）为 'cmd'|'ctrl' 模式二选一、默认按系统；
   * 现改为完整组合串模型：'Ctrl+Shift+X' / 'Alt+Shift+E' / 'Command+Shift+X' 等，
   ...
   * 未设置 / 非法值统一回退默认 Ctrl+Shift+X（不再按操作系统区分）。
   */
  async getElementPickerShortcut() {
    ...
    return this.normalizeShortcut(v) || this.SHORTCUT_DEFAULT;
  },
   ...
   * 须用 'MacCtrl'（manifest 的 mac 键位同规则，见 source-driven 文档核对）。
   */
