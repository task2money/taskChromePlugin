...
const Storage = {
  ...
  /**
   * 解析操作系统默认的快捷键修饰键：
   * macOS → 'cmd'（⌘+Shift+X），Windows/Linux 等 → 'ctrl'（Ctrl+Shift+X）。
   * @param {object|null} [nav] 可注入的 navigator（测试传 mock；缺省取全局 navigator）
   * @returns {'cmd'|'ctrl'}
   */
  detectOsShortcut(nav) {
    ...
    return plat.includes('mac') ? 'cmd' : 'ctrl';
  },

  /** 获取元素拾取快捷键配置 — 未设置 / 非法值回退操作系统默认 */
  async getElementPickerShortcut() {
    const cfg = await this.get(['elementPickerShortcut']);
    const v = cfg.elementPickerShortcut;
    return v === 'cmd' || v === 'ctrl' ? v : this.detectOsShortcut();
  },

  /** 保存元素拾取快捷键配置（非法值归一化为操作系统默认） */
  async saveElementPickerShortcut(mode) {
    const normalized = mode === 'cmd' || mode === 'ctrl' ? mode : this.detectOsShortcut();
