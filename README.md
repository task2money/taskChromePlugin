...
### 0.6 页面优化建议（Alt+`）
- 登录后在扩展弹窗「Alt+` 默认上下文」选择默认工作空间/项目（或浮窗选择），再按 **Alt+`** → 采集可见页文与 DOM 大纲并请求优化建议
- **Alt+Shift+`**：先点选页面元素，再仅对该元素（或多选）生成优化建议（流程同 Alt+`）
- 建议以锚定悬浮卡展示（无遮罩）；勾选可临时预览；「全部填入任务描述 / 逐条填入任务描述」追加写入任务描述（不自动创建任务）；无智能体资源时提示配置入口
...
│   ├── sw-page-advisor.js         # Alt+` 页面优化建议：建 job + 轮询
