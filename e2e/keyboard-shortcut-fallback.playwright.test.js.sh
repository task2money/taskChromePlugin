#!/usr/bin/env bash
# 运行快捷键（Cmd/Ctrl+Shift+X）页内兜底 E2E（Playwright）
set -euo pipefail
...
"$PW_BIN" test -c e2e/playwright.config.js e2e/keyboard-shortcut-fallback.playwright.test.js --timeout=60000
echo "keyboard-shortcut-fallback.playwright.test.js: OK"
